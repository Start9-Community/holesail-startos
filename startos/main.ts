...
// Reactive daemon set: one Holesail server daemon per stored tunnel. The store
// `.const` is read INSIDE the reconciler's builder, where `constRetry` drives a
// reconcile rather than firing `effects.restart()` — so adding, removing, or
// editing a single tunnel touches only that daemon and every other tunnel keeps
// running. Reading the same `.const` in the `setupMain` body instead would
// restart the WHOLE service on any store change, dropping every tunnel.
export const main = sdk.setupMain(async ({ effects }) =>
  sdk.Daemons.dynamic(effects, async ({ effects }) => {
