...
This repo packages [Holesail](https://github.com/holesail/holesail-docker) for StartOS.
...
Holesail runs the `holesail/holesail:latest` image — the manifest's `dockerTag` is not pinned to a specific upstream version, so a "bump" is purely a StartOS package-version change to record that a new upstream release has been pulled.

1. Update `version` and `releaseNotes` in the file under `startos/versions/`, renaming it to the new version string. A *new* version file is only needed when the bump carries an `up`/`down` migration, or when you want the old release notes preserved in git history — see [Versions](https://docs.start9.com/packaging/versions.html).
2. Rebuild (`make`), sideload the `.s9pk`, and confirm it starts.
3. Review `README.md` and `instructions.md` for anything the bump changed.
